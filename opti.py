...
class OptiDrone:
    ...
    def eval(self, time = 60, goal = [5.0, 5.0, 5.0, 0.0, 0.0, 0.0]):
        ...
        self.opti.subject_to(self.u[0, :] <= self.max_force)
        self.opti.subject_to(self.u[1, :] <= self.max_force)
        self.opti.subject_to(self.u[2, :] <= self.max_force)
        self.opti.subject_to(self.u[0, :] >= -self.max_force)
        self.opti.subject_to(self.u[1, :] >= -self.max_force)
        self.opti.subject_to(self.u[2, :] >= -self.max_force)
    ...
    def animate(self):
        ...
        x = self.sol.value(self.x)[0, :]
        y = self.sol.value(self.x)[1, :]
        z = self.sol.value(self.x)[2, :]
        ...
        self.ani = animation.FuncAnimation(fig, update_points, num_of_frames, interval=10, repeat=True)
        plt.show()
